...
to draw-centers
  let erasing? any? centers with [xcor = floor mouse-xcor + 0.5 and ycor = floor mouse-ycor + 0.5]
  while [mouse-down?] [
    ifelse erasing? [
      ask centers with [xcor = floor mouse-xcor + 0.5 and ycor = floor mouse-ycor + 0.5] [die]
    ] [
    set-default-shape centers "flag"
    create-centers 1 [
      set-centers
      setxy floor mouse-xcor + 0.5 floor mouse-ycor + 0.5
    ]
    display
    ]
  ]
  ask centers [ifelse any? centers != 0 [set centers? true][set centers? false]]
end
